        ...
        <div className="space-y-6 max-w-6xl">
            ...
            {groups.length === 0 ? (
                <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-16 text-center">
                    ...
                    <p className="text-sm font-medium text-slate-400">{t("Aucune affectation. L'administration doit vous affecter à des matières.")}</p>
                </div>
            ) : (
                    ...
                    {groups.map((g, gi) => (
                        <motion.div key={g.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: gi * 0.05 }}
                            ...
                            <div className="px-5 py-4 bg-gradient-to-r from-slate-900 to-slate-800 text-white">
                                ...
                                <p className="text-xs text-slate-300">{g.niveau} · {g.items.length} {t('matière(s)')}</p>
                            </div>
                                    ...
                                    <div key={a.id} className="flex items-center gap-3 px-5 py-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-slate-400 flex items-center gap-1">
                                                <Hash className="w-3 h-3" /> {t('coef.')} {a.coefficient ?? a.matiere.coefficient}
                                            </p>
                                        </div>
                                        <div className="flex flex-col sm:flex-row gap-1.5">
                                            <button onClick={() => navigate(`/prof/grades?classe_id=${a.classe.id}&matiere_id=${a.matiere.id}`)}
                                                ...
                                                <ClipboardList className="w-3.5 h-3.5" /> {t('Notes')}
                                            </button>
                                            <button onClick={() => navigate(`/prof/attendance?classe_id=${a.classe.id}&matiere_id=${a.matiere.id}`)}
                                                ...
                                                <UserCheck className="w-3.5 h-3.5" /> {t('Appel')}
                                            </button>
                                        </div>
                                    </div>
                                ))}
