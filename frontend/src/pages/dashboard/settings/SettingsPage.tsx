    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6">
            <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Configuration de votre établissement et de votre compte</p>
            </div>
...
function SchoolTab() {
    ...
    const handleLogoFile = async (file?: File) => {
        ...
        try {
            ...
            setMsg({ type: 'err', text: e?.response?.data?.error ?? e?.message ?? 'Erreur lors de l\'upload du logo.' });
    ...
    const handleSetYear = async () => {
        ...
        try {
            ...
            setYearMsg({ type: 'err', text: err?.response?.data?.error ?? 'Erreur.' });
        } finally {
            setSavingYear(false);
        }
    };
        ...
        <div className="space-y-5">
            ...
            {tenant && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 grid grid-cols-2 gap-3 text-sm">
                    <InfoRow label="Domaine"  value={tenant.sous_domaine} />
                    <InfoRow label="Plan"     value={<PlanBadge plan={tenant.plan_abonnement} />} />
                    <InfoRow label="Devise"   value={tenant.devise} />
                    {tenant.date_expiration && (
                        <InfoRow label="Expiration" value={new Date(tenant.date_expiration).toLocaleDateString('fr-FR')} />
                    )}
                </div>
            )}

            {/* Année active */}
            <Card title="Année scolaire active" icon={<Calendar size={16} />}>
                {yearMsg && <Feedback msg={yearMsg} />}
                <div className="flex gap-3 items-end">
                    <div className="flex-1">
                        <label className="text-xs font-medium text-slate-500 mb-1 block">Année courante</label>
                    ...
                    <button onClick={handleSetYear} disabled={savingYear || !activeYear}
                        ...
                        {savingYear ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Appliquer
                    </button>
                </div>
            </Card>

            {/* Infos école */}
            <Card title="Informations de l'établissement" icon={<Building2 size={16} />}>
                {msg && <Feedback msg={msg} />}
                <div className="grid grid-cols-2 gap-4">
                    <Field label="Nom de l'établissement *" className="col-span-2">
                    ...
                    <Field label="Logo de l'établissement" className="col-span-2">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap gap-2">
                                    <label className="inline-flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 cursor-pointer transition-all">
                                        ...
                                        {form.logo_url ? 'Changer' : 'Téléverser un logo'}
                                ...
                                <p className="text-[11px] text-slate-400 mt-1.5">PNG, JPG ou WebP · max 4 Mo · apparaît sur les bulletins.</p>
                            </div>
                        </div>
                    </Field>
                </div>
...
function ProfileTab() {
    ...
    return (
        <Card title="Mon profil" icon={<User size={16} />}>
            {msg && <Feedback msg={msg} />}
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Langue d'interface" className="col-span-2">
                    <div className="relative">
                ...
                {profile?.created_at && (
                    <Field label="Membre depuis" className="col-span-2">
...
function PasswordTab() {
    ...
    return (
        <Card title="Changer de mot de passe" icon={<Lock size={16} />}>
            {msg && <Feedback msg={msg} />}
            ...
            <div className="flex justify-end pt-2">
                <button onClick={handleSave} disabled={saving || !form.current || !form.next || !form.confirm}
                    className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white text-sm font-semibold rounded-xl hover:bg-emerald-700 disabled:opacity-60">
                    {saving ? <Loader2 size={14} className="animate-spin" /> : <Lock size={14} />} Mettre à jour
                </button>
            </div>
        </Card>
    );
